        ...
        <div class="modal-content" style="background: white; max-width: 900px; margin: 20px auto; border-radius: 16px; box-shadow: 0 25px 50px rgba(0,0,0,0.25); position: relative;">
            ...
            <div style="padding: 24px; max-height: calc(100vh - 200px); overflow-y: auto;">
                ...
                <form id="planningForm">
                    ...
                    <!-- Compatibility hook used by existing Segments/Budget/Schedule helpers. -->
                    <select id="active_campaign" aria-hidden="true" tabindex="-1" style="display:none"></select>
...
function renderAiRecReportsTab(body, data, rec = {}) {
    ...
    // Hide technical missing-source snapshot messages from the Supporting Reports UI.
    // Older recommendations may still have these messages stored in planning_error_message.
    const rawWarning = summary.planning_error_message || reports.warning || '';
    const warning = String(rawWarning)
        .replace(/Supporting report\s+\S+\s+could not be fully restored from the source database\.\s*/gi, '')
        .trim();
...
async function manualPersistPlan(complete=false){
    ...
    try{
        ...
        if(!manualPlannerState.campaignId){summary.status='draft';result=await manualApiJson(apiBase+'/api/v1/campaigns',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(summary)});manualPlannerState.campaignId=Number(result.id);manualPlannerState.mode='edit';document.getElementById('planningForm').dataset.campaignId=manualPlannerState.campaignId;}
            ...
            // Preserve the existing workflow status. Sending the same status back can still be
            // rejected for some roles (for example Staff editing a Pending campaign).
            const updateSummary = {...summary};
            delete updateSummary.status;
            result=await manualApiJson(apiBase+`/api/v1/campaigns/${manualPlannerState.campaignId}`,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(updateSummary)});
        }
...
async function openManualPlannerForCampaign(campaignId,mode='edit'){
    try{
        ...
        activeCampaignId = Number(campaignId);
        const legacyCampaignSelect = document.getElementById('active_campaign');
        if (legacyCampaignSelect) {
            if (!Array.from(legacyCampaignSelect.options).some(o => Number(o.value) === Number(campaignId))) {
                const opt = document.createElement('option'); opt.value = String(campaignId); opt.textContent = String(campaignId); legacyCampaignSelect.appendChild(opt);
            }
            legacyCampaignSelect.value = String(campaignId);
        }
...
function clearBudgetForm() {
    const defaults = {
        budget_campaign_id: '', budget_item_name: '', budget_item_type: 'consumable',
        budget_quantity: '1', budget_unit_cost: '', budget_funding_source: 'government_allocated', budget_notes: ''
    };
...
function onCampaignChange() {
    const legacyCampaignEl = document.getElementById('active_campaign');
    activeCampaignId = parseInt(legacyCampaignEl?.value || '0', 10);
    loadSegments();
}
...
async function loadSegments() {
    const legacyCampaignEl = document.getElementById('active_campaign');
    const cid = activeCampaignId || parseInt(legacyCampaignEl?.value || '0', 10);
    if (!cid) {
